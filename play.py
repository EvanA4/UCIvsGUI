...
class GameBoard:
    ...
    def __init__(self, engineDir, defaultFullscreen, startingPlayerTeam, startingTime, incTime, screen):
        ...
        # Want a unique initial board? Look at these commands for references:
        ...
        # self.board.set_fen("2kr3r/1ppq2pp/p1pbbpn1/8/N2PP3/5NBP/PP3PP1/2RQ1RK1 b - - 10 14") # fancy looking board
...
def print_usage():
    ...
    print("flags:\n" +
          ...
          "-d [name]       :           displayed name of application\n" +
...
def handle_args(args):
    prevArgs = [False, False, False, False, False, False]

    displayName = "UCIvsGUI"
    ...
    for i in range(1, len(args)):
        if args[i] == "-fs":
            ...
            if (args[i - 1] != "-time" and args[i - 1] != "-inc" and args[i - 1] != "-e" and args[i - 1] != "-d"):
                print_usage()
    ...
    return engineDir, displayName, defaultFullscreen, startingPlayerTeam, startingTime, incTime


# initializes board and both asynch functions
def main():
    engineDir, displayName, defaultFullscreen, startingPlayerTeam, startingTime, incTime = handle_args(sys.argv)
    ...
    for file in os.listdir(dir):
        svg2png(url="svgs/" + os.fsdecode(file), write_to="pngs/" + os.fsdecode(file)[:-3] + "png", output_height=board.width // 8, output_width=board.width // 8)
    board.draw()
